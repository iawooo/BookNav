    ...
    <script>
        ...
        async function fetchFavicon() {
            ...
            try {
                ...
                if (html) {
                    ...
                    const icon = extractIconsFromHtml(html, baseUrl);
                    if (icon) {
                        fetchedIcon.value = icon;
                        logDebug(`✓ Found valid icon: ${icon}`);
                        return;
                    }

                    // Step 3: Check Web App Manifest
                    logDebug("Looking for Web App Manifest...");
                    const manifestUrl = findManifestUrl(html, baseUrl);
                    if (manifestUrl) {
                        const manifest = await fetchContent(manifestUrl) || await fetchContent(`https://api.allorigins.win/raw?url=${encodeURIComponent(manifestUrl)}`);
                        if (manifest) {
                            const manifestIcon = await fetchManifestIcon(manifest, baseUrl);
                            if (manifestIcon) {
                                fetchedIcon.value = manifestIcon;
                                logDebug(`✓ Found manifest icon: ${manifestIcon}`);
                                return;
                            }
                        }
                    ...
                    if (browserConfig) {
                        const tileIcon = extractTileIcon(browserConfig, baseUrl);
                        if (tileIcon) {
                            fetchedIcon.value = tileIcon;
                            logDebug(`✓ Found tile icon: ${tileIcon}`);
                            return;
                        }
                        logDebug("⚠ No tile images found in browserconfig.xml");
                    } else {
                        logDebug("⚠ Direct XML fetch failed or no browserconfig.xml");
                    }
                } else {
                    logDebug("⚠ Failed to fetch HTML");
                }

                // Step 5: Fallback to Google FaviconV2
                const googleFavicon = `https://t0.gstatic.com/faviconV2?client=SOCIAL&type=FAVICON&fallback_opts=TYPE,SIZE,URL&url=${encodeURIComponent(url)}&size=64`;
                logDebug(`Trying Google FaviconV2 as fallback: ${googleFavicon}`);
                if (await checkImage(googleFavicon)) {
                    fetchedIcon.value = googleFavicon;
                    logDebug(`✓ Found Google FaviconV2: ${googleFavicon}`);
                    return;
                }

                fetchedIcon.value = '';
                logDebug("⚠ No icons found");
            } catch (e) {
                fetchedIcon.value = '';
        ...
        function findManifestUrl(html, baseUrl) {
            ...
            if (link && link.getAttribute('href')) {
                return resolveUrl(link.getAttribute('href'), baseUrl);
            }
            return null;
        }

        async function fetchManifestIcon(manifest, baseUrl) {
            try {
        ...
        function extractTileIcon(xml, baseUrl) {
            const parser = new DOMParser();
            const doc = parser.parseFromString(xml, 'text/xml');
            const tiles = doc.querySelectorAll('[src]');
            let bestIcon = null;
            let maxSize = 0;
